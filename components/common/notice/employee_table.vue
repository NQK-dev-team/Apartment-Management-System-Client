<template>
  <a-table
    ...
    :data-source="data"
    :row-selection="{
      selectedRowKeys: notificationData.staffReceivers,
      onChange: (selectedRowKeys: any) => {
        notificationData.staffReceivers = selectedRowKeys;
      },
      getCheckboxProps: (record: any) => ({
        id: `check_staff_${record.no}`,
        name: `check_staff_${record.no}`,
      }),
    }"
    class="mt-3"
  ...
  >
    <template #bodyCell="{ value, column, record }">
      <template v-if="column.key === 'action'">
        <NuxtLink
          :id="`staff_${record.no}_detail_link`"
          :name="`staff_${record.no}_detail_link`"
