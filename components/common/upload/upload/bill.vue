...
async function fetchUploadedFiles() {
  try {
    $event.emit('loading');
    data.value = [];
    const response = await api.common.upload.getUploadFiles(COMMON.UPLOAD_TYPE.ADD_BILLS);
...
async function uploadFile(file: UploadFile) {
  try {
    ...
    formdata.append('uploadType', COMMON.UPLOAD_TYPE.ADD_BILLS.toString());
...
onMounted(() => {
  ...
  websocketConnection.value.onmessage = (event) => {
    ...
    if (data.type === COMMON.WEBSOCKET_SIGNAL_TYPE.UPLOAD_BILL) {
      if (timeOut.value) {
