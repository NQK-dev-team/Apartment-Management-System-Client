...
useHead({
  ...
  meta: [
    {
      name: 'description',
      content: 'View and manage user important notifications',
    },
  ],
});
...
async function getInboxList(emitLoading = true) {
  try {
    ...
    if (offset.value === 0) {
      inboxList.value = response.data.filter((elem) => elem.isMarked === COMMON.NOTIFICATION.MARKED);
    } else {
      inboxList.value.push(...response.data.filter((elem) => elem.isMarked === COMMON.NOTIFICATION.MARKED));
    }
...
onMounted(() => {
  ...
  websocketConnection.value.onmessage = (event) => {
    ...
    if (data.type === COMMON.WEBSOCKET_SIGNAL_TYPE.NEW_IMPORTANT) {
      if (offset.value === 0) {
        getInboxList(false);
      } else {
        offset.value = 0;
