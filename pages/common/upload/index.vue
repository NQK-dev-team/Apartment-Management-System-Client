  ...
  <div class="w-full h-full flex flex-col px-5">
    ...
    <div class="flex-1 flex flex-col px-4 mt-5" :class="[lightMode ? 'bg-white' : 'bg-[#1f1f1f] text-white']">
      <div class="mt-5">
        <div>
          ...
          <a-radio-group v-model:value="importOption">
            <a-radio-button v-for="value in Object.values(COMMON.IMPORT_TYPE)" :key="value" :value="value">{{
              $t(getImportTypeLabel(value))
            }}</a-radio-button>
          </a-radio-group>
        </div>
        <div class="mt-5">
      ...
      <div v-if="viewCatory === 1" class="flex-1">
        <CommonUploadUploadCustomer v-if="importOption === COMMON.IMPORT_TYPE.ADD_CUSTOMERS" />
        <CommonUploadUploadContract v-if="importOption === COMMON.IMPORT_TYPE.ADD_CONTRACTS" />
        <CommonUploadUploadBill v-if="importOption === COMMON.IMPORT_TYPE.ADD_BILLS" />
      </div>
      <div v-else-if="viewCatory === 2" class="flex-1">
        <CommonUploadResultCustomer v-if="importOption === COMMON.IMPORT_TYPE.ADD_CUSTOMERS" />
        <CommonUploadResultContract v-if="importOption === COMMON.IMPORT_TYPE.ADD_CONTRACTS" />
        <CommonUploadResultBill v-if="importOption === COMMON.IMPORT_TYPE.ADD_BILLS" />
      </div>
    </div>
  </div>
</template>
