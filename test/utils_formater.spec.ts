...
describe('convertToDateTime2', () => {
  beforeEach(() => {
    // Mock toLocaleDateString to ensure consistent results
    vi.spyOn(Date.prototype, 'toLocaleDateString').mockImplementation((locale, options) => {
      if (locale === 'vi-VN') {
        return 'Thứ Hai, 25 tháng 12, 2023';
      } else {
        return 'Monday, December 25, 2023';
      }
    });
  });

  it('formats date with Vietnamese locale', () => {
    const result = convertToDateTime2('2023-12-25T10:30:45Z', 'vi');
    expect(result).toContain('Thứ Hai, 25 tháng 12, 2023');
    expect(result).toMatch(/\d{2}:\d{2}:\d{2}$/); // Should end with time
  });

  it('formats date with English locale', () => {
    const result = convertToDateTime2('2023-12-25T10:30:45Z', 'en');
    expect(result).toContain('Monday, December 25, 2023');
    expect(result).toMatch(/\d{2}:\d{2}:\d{2}$/); // Should end with time
  });

  it('includes time in HH:mm:ss format', () => {
    const result = convertToDateTime2('2023-12-25T10:30:45Z', 'en');
    expect(result).toMatch(/ 10:30:45$/);
  });

  it('pads time values with zeros', () => {
    const result = convertToDateTime2('2023-12-25T09:05:05Z', 'en');
    expect(result).toMatch(/ 09:05:05$/);
  });

  it('handles midnight', () => {
    const result = convertToDateTime2('2023-12-25T00:00:00Z', 'en');
    expect(result).toMatch(/ 00:00:00$/);
  });

  it('handles different locales consistently', () => {
    const dateStr = '2023-12-25T15:30:45Z';
    const viResult = convertToDateTime2(dateStr, 'vi');
    const enResult = convertToDateTime2(dateStr, 'en');
    
    // Both should end with the same time
    expect(viResult).toMatch(/ 15:30:45$/);
    expect(enResult).toMatch(/ 15:30:45$/);
    
    // But have different date formats
    expect(viResult).toContain('Thứ Hai, 25 tháng 12, 2023');
    expect(enResult).toContain('Monday, December 25, 2023');
  });

  it('handles edge case with unknown locale (defaults to English)', () => {
    const result = convertToDateTime2('2023-12-25T10:30:45Z', 'unknown');
    expect(result).toContain('Monday, December 25, 2023');
    expect(result).toMatch(/ 10:30:45$/);
  });
});